...
from convtts_slr.screening import Decider, verdict_from, route
from convtts_slr.sources import ArxivSource, LocalSource, OpenAlexSource, SemanticScholarSource, openalex_abstract
...
def test_dedup_merges_preprint_and_venue_version():
    pre = Paper(id="a", title="DailyTalk: Spoken Dialogue Dataset for Conversational Text-to-Speech",
                doi="10.48550/arXiv.2207.01063", year=2022, sources=["arxiv"])
    venue = Paper(id="b", title="DailyTalk: spoken dialogue dataset for conversational text-to-speech",
                  doi="https://doi.org/10.1109/ICASSP49357.2023.10095751", year=2023, venue="ICASSP",
                  sources=["openalex"])
...
def test_routing_and_seed_alarm():
    ...
    scores = {"ic4_pitch_predictor": 0.05, "ic4_energy_predictor": 0.05, "ic4_duration_predictor": 0.95}
...
def test_quote_grounding():
    ...
    facts = DatasetFacts(total_hours=Quoted(value=20.0, quote="We recorded 20 hours of dialogue"),
                         num_speakers=Quoted(value=10), languages=Quoted(value="en", quote="All speech is English."))
...
def test_source_parsers():
    ...
    assert '"text-to-speech"' in OpenAlexSource.render(cfg) and " AND " in OpenAlexSource.render(cfg)
    ...
    assert p.arxiv_id == "2207.01063v2" and p.year == 2022 and p.title == "DailyTalk: Spoken Dialogue Dataset"
...
def test_jev_backend_translates_specs(monkeypatch):
    ...
    def fake_system_one(self, state, questions, **kw):
        seen["q"] = questions
        return SystemOneResponse.model_validate({"model": "jev-latest", "usage": {}, "answers": {
            "ic1_new_dataset": {"type": "noul", "noul": 0.97},
            "rq1_sample_unit": {"type": "choice", "choice": "turn_with_context", "confidence": 0.8,
                                "probabilities": {"utterance_only": 0.05, "turn_with_context": 0.8,
                                                  "full_dialogue": 0.1, "session_multichannel": 0.03,
                                                  "unclear": 0.02}}}})
    ...
    assert isinstance(seen["q"]["ic1_new_dataset"], Noul) and isinstance(seen["q"]["rq1_sample_unit"], Choice)
...
CORPUS = [
    {"title": "DailyTalk: Spoken Dialogue Dataset for Conversational Text-to-Speech", "arxiv_id": "2207.01063",
     "year": 2022, "abstract": "We introduce DailyTalk, a high-quality conversational speech dataset for "
     "conversational text-to-speech, recorded by two speakers from dialogues."},
    {"title": "EmoDialog: an emotional two-speaker dialogue corpus for conversational TTS", "year": 2024,
     "doi": "10.1/emodialog", "abstract": "We release a new dialogue speech corpus with emotion labels for "
     "conversational text-to-speech."},
    {"title": "A better context encoder for conversational TTS", "year": 2023, "doi": "10.1/model",
     "abstract": "We propose a model for conversational text-to-speech evaluated on existing datasets."},
    {"title": "CallASR: a telephone dialogue corpus for speech recognition", "year": 2023, "doi": "10.1/asr",
     "abstract": "We release a dialogue speech corpus for automatic speech recognition only."},
    {"title": "ReadBook: a read-speech corpus for text-to-speech", "year": 2024, "doi": "10.1/read",
     "abstract": "We release a new read speech corpus of a single speaker for text-to-speech."},
    {"title": "OldDialog: a 2015 dialogue speech corpus for TTS", "year": 2015, "doi": "10.1/old",
     "abstract": "We release a dialogue speech corpus for conversational text-to-speech."},
    {"title": "PodGen: a new dialogue speech collection", "year": 2025, "doi": "10.1/podgen",
     "abstract": "We release a dialogue corpus maybe for speech things."},
]

FULLTEXT = {
    "arxiv:2207.01063": "3 Dataset\nEach sample is a turn with its dialogue context. Speech was recorded from "
                        "scripts read by two speakers. 4 Model\nFastSpeech 2 with a pitch predictor and an "
                        "energy predictor. We recorded 20 hours of dialogue.",
    "doi:10.1/emodialog": "2 Corpus\nEach sample is a turn with its dialogue context. Emotion labels. Speech was "
                          "recorded spontaneously. 3 Baseline\nFastSpeech 2 with a pitch predictor. "
                          "We recorded 12 hours of dialogue.",
    "doi:10.1/snowkid": "2 Data\nEach sample is a full dialogue. Harvested from podcasts. 3 Model\nA duration "
                        "predictor only. The corpus has 300 hours of dialogue.",
}
...
class FakeCitations:
    ...
    def references(self, p):
        if p.id == "doi:10.1/emodialog":
            return [Paper(id="x", title="SnowKid: a podcast dialogue corpus for conversational text-to-speech",
                          doi="10.1/snowkid", year=2025,
                          abstract="We release a dialogue corpus for conversational text-to-speech.")]
        return [Paper(id="y", title="DailyTalk: Spoken Dialogue Dataset for Conversational Text-to-Speech",
                      arxiv_id="2207.01063", year=2022)]  # a duplicate: must not create a new record
...
def make_ctx(tmp_path, protocol=DEFAULT_PROTOCOL, script=keyword_script):
    ...
    ctx = Context(protocol=protocol, store=store, decider=Decider(backend, store),
                  sources=[LocalSource(corpus, name="fixture")], citation_sources=[FakeCitations()],
                  seeds=LocalSource(SEEDS, name="seeds").search(), facts=ScriptedFactsExtractor(facts_fn),
                  checker=Decider(ScriptedBackend(script), store), workers=3,
                  fetch=fetch, to_text=lambda f: f.read_text())
    return ctx, backend
...
def test_end_to_end(tmp_path):
    ...
    assert ta[behavior] == "needs_human"  # seed with no abstract: recall alarm, not a silent exclusion
    ...
    assert flow["included"] == 3 and flow["reference_baselines"] == 4 and flow["snowball_rounds"] == 2
